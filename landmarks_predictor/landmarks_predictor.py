...
def _find_predictor(default_name: str) -> Optional[str]:
...
def get_predictor(predictor_fp: Optional[str], auto_download: bool) -> str:
    ...
    if predictor_fp is None:
        predictor_fp = _find_predictor("shape_predictor_68_face_landmarks.dat")
        if predictor_fp is None:
...
class LandmarkPredictor:
    ...
    def forget_landmarks(self) -> None:
        """Resets landmarks collection, helpful in single image detection."""
        self._landmarks_collection = {}
        self.fake_map = {}

    @classmethod
    def _display_sample(
        cls, image: Image, rect: dlib.rectangle, shape: full_object_detection
    ) -> None:
    ...
    def get_landmarks(self) -> Dict[str, Dict[int, Tuple[int, int]]]:
        """Returns landmarks collection with a map filename:landmarks_list"""
